...
WITH seating_types AS (
   SELECT DISTINCT
       ...
       CASE
           WHEN UPPER(TRIM(approved_for_sidewalk_seating)) IN ('YES', 'Y', 'TRUE', 'APPROVED') THEN TRUE
           ELSE FALSE
       END AS approved_for_sidewalk,

       CASE
           WHEN UPPER(TRIM(approved_for_roadway_seating)) IN ('YES', 'Y', 'TRUE', 'APPROVED') THEN TRUE
           ELSE FALSE
